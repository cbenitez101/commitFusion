        ...
        case 'dispositivos':
            if ($_SESSION['cliente'] == 'admin') {
                if (!empty($template_data[2])) {
                    $result = $database->query("SELECT syslog.*, dispositivos.notas, dispositivos.tipo, hotspots.ServerName FROM `dispositivos` INNER JOIN hotspots ON dispositivos.id_hotspot = hotspots.id INNER JOIN syslog ON hotspots.ServerName = syslog.local WHERE dispositivos.id_hotspot='".$template_data[2]."' AND (dispositivos.descripcion = syslog.dispositivo OR syslog.dispositivo = 'hotspot') GROUP BY id");
                    $out = array();
                    if ($result->num_rows > 0) {
                        ...
                        foreach ($out as $value) {
                            $aux1 = array('estado' => '<button type="button" class="btn btn-'.((strtotime($value['fecha']) < strtotime("-30 min"))?"danger":"success").' btn-circle"><i class="fa fa-'.((strtotime($value['fecha']) < strtotime("-30 min"))?"times":"check").'"></i>','fecha' => $value['fecha'], "ip"=> $value['ip'], "dispositivo" => $value['dispositivo'], "notas" => $value['notas']);
                            if (isset($value['info'])) {
